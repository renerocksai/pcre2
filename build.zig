...
pub fn build(b: *std.Build) !void {
    ...
    const pcre2_header = pcre2_header_dir.addCopyFile(upstream.path("src/pcre2.h.generic"), "pcre2.h");
    ...
    lib.addCSourceFile(.{
        .file = b.addWriteFiles().addCopyFile(upstream.path("src/pcre2_chartables.c.dist"), "pcre2_chartables.c"),
    });
    ...
    if (codeUnitWidth == CodeUnitWidth.@"8") {
        ...
        posixLib.addIncludePath(upstream.path("src"));
        ...
        posixLib.installHeader(upstream.path("src/pcre2posix.h"), "pcre2posix.h");
    ...
    pcre2test.addIncludePath(upstream.path("src"));

    pcre2test.addCSourceFile(.{
        .file = upstream.path("src/pcre2test.c"),
    });
